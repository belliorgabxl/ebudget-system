...
import { MOCK_APPROVALS } from "@/app/mock";
...
export default function UserDashboardPage() {
  ...
  const [calendar_events_data, set_calendar_events_data] = useState<
    GetCalenderEventRespond[]
  >([]);
  const [approval_items, set_approval_items] = useState<GetApprovalItems[]>([]);
  const [qa_indicators_data, set_qa_indicators_data] = useState<
    GetQaIndicatorsByYearAllRespond[]
  >([]);
  const [strategic_plans_data, set_strategic_plans_data] = useState<
    GetStrategicPlanRespond[]
  >([]);
  ...
  useEffect(() => {
    const fetchCounts = async () => {
      try {
        const res = await GetProjectsByOrgFromApi({ page: 1, limit: 10 }); 
         set_projects_data(res?.data ?? []);
      } catch (err) {
        console.error(err);
      }
    };
  ...
  useEffect(() => {
    const fetchQaIndicators = async () => {
      try {
        ...
        const data = await GetQaIndicatorsByYearAllFromApi(
